...
{   FILE *fpr;
    ...
    char s1[10],s2[10],s3[10],s4[]="/",s[60];
    sprintf(s1,"%d",day);
    sprintf(s2,"%d",month);
    sprintf(s3,"%d",year);
    if(month<=9)
        {sprintf(s,"%s/0%s/%s",s1,s2,s3);}
    else
    {
        sprintf(s,"%s/%s/%s",s1,s2,s3);
    }
    printf("TODAY'S DATE: %s",s);
    printf("\nTODAY'S EVENTS: ");
    while(fread(&d1,sizeof(d1),1,fpr))
    {   
        if(strcmp(d1.date,s)==0)
        { 
            printf("\nName: %s \nEvent: %s \n",d1.name,d1.event);
            flag=1;
        }               
